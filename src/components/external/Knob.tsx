...
const Knob = ({
	...
	const handleUpdate = (mouseY) => {
		const newRotation = clamp(
			rotationRef.current - (mouseY - currentY),
			-maxRotation,
			maxRotation
		);
	...
	const getDashOffset = (rotation, type) =>
		typeInfo[type].dashLength -
		(184 / (maxRotation * 2)) * (rotation + typeInfo[type].offset);
		...
		<ComponentContainer className={`${BASE_CLASS_NAME}`} disabled={disabled}>
			...
			<Label disabled={disabled}>
				...
				<div className="value-text">
					{Math.round(
						getValueFromKnobRotation(rotation) * 10 ** decimalPlaces
					) /
						10 ** decimalPlaces}
				</div>
			</Label>
